// Simple Supabase Admin Client - KISS approach
let supabaseAdmin;

// Zentro Homes Admin Dashboard with Integrated Media Upload
// CLEANED UP VERSION - Removed duplicates, focused on property updates
class ZentroAdminIntegrated {
  constructor() {
    this.properties = [];
    ...
    // Media management - KISS approach with multiple images support
    this.selectedMedia = {
  ...
  async waitForSupabase() {
    ...
    while (!supabaseManager && attempts < maxAttempts) {
  ...
  async loadPropertiesFromDatabase() {
    ...
    try {
      ...
      this.properties = await supabaseManager.getAllProperties();
      this.filteredProperties = [...this.properties];
  ...
  // Legacy bindMediaEvents method - replaced with KISS approach
  bindMediaEvents() {
    console.log('Legacy bindMediaEvents called - using newer methods instead');
  }

  // Legacy drop zone and file selection methods removed - using KISS approach

  // Legacy handleImageUrl method removed - using new approach
  ...
  // Legacy main image methods removed - using multiple images approach


  // Legacy removeVideo method - removed to avoid duplicates
  ...
  openPropertyModal(property = null) {
    ...
    setTimeout(() => {
      console.log('🎬 Binding media events for modal...');
      this.bindMultipleImageEvents();
      this.bindYouTubeEvents();
      console.log('✅ Media events bound for modal');
    }, 200);
  }
  ...
  populatePropertyForm(property) {
    ...
    console.log('✅ Property form populated for editing:', property.title);
  }
  ...
  getFormData() {
    // For the form data, we don't include image URLs since they're handled separately in processMediaUploads
  ...
  async saveProperty() {
    ...
    try {
      ...
      if (this.currentEditingId && this.currentEditingId !== 'new') {
        ...
        await supabaseManager.updateProperty(this.currentEditingId, formData, mediaData);
        this.showNotification('Property updated successfully!', 'success');
      } else {
        // Add new property with media
        await supabaseManager.addProperty(formData, mediaData);
  ...
  handleMultipleImages(files) {
    ...
    console.log(`Selected ${files.length} images. First will be main image.`);
  }
  ...
  bindYouTubeEvents() {
    ...
    console.log('Binding YouTube URL events...', {
    ...
    youtubeInput.addEventListener('input', (e) => {
      ...
      console.log('YouTube URL updated:', this.selectedMedia.youtubeUrl);
    });
    ...
    console.log('✅ YouTube URL events bound successfully');
  }
  ...
  // Process media uploads and prepare data for database
  // Simple validation - KISS approach
  ...
  async processMediaUploads() {
    ...
    // Process multiple images - first one is main, rest are gallery
    if (this.selectedMedia.images && this.selectedMedia.images.length > 0) {
      console.log(`📸 Processing ${this.selectedMedia.images.length} images...`);
      
      const imagePromises = this.selectedMedia.images.map(async (imageFile, index) => {
        const storageUrl = await this.uploadToStorage(imageFile, 'image');
  ...
  testPropertyUpdate() {
    ...
    console.log('- Selected images:', this.selectedMedia.images.length);
    console.log('- YouTube URL:', this.selectedMedia.youtubeUrl);
    console.log('- Supabase manager available:', !!supabaseManager);
    
    if (this.currentEditingId && this.currentEditingId !== 'new') {
      console.log('✅ Ready for property update');
    } else {
      console.log('✅ Ready for new property creation');
    }
  }
  ...
  async uploadToStorage(file, fileType = 'image') {
    try {
      // Simple admin client creation - KISS approach
      if (!supabaseAdmin) {
        const serviceKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inlxc2tsZHNrZW9rdmdpZ3lyZm53Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MzY4NDQwMywiZXhwIjoyMDY5MjYwNDAzfQ.51ZXyxnjeXUhrrufQrnZOh5-uo67N8Za6Qsz3Bm1TF4';
        supabaseAdmin = window.supabase.createClient(
          'https://yqskldskeokvgigyrfnw.supabase.co',
          serviceKey
        );
        console.log('✅ Admin client created for storage upload');
      }
  ...
  // Rest of the methods remain the same as in admin-supabase.js
  async filterByStatus(status) {
    ...
    try {
      this.setLoading(true);
      this.filteredProperties = await supabaseManager.getPropertiesByStatus(status);
      this.renderProperties();
    } catch (error) {
  ...
  async filterProperties() {
    ...
    try {
      this.setLoading(true);
      this.filteredProperties = await supabaseManager.searchProperties(search);
      this.renderProperties();
    } catch (error) {
  ...
  async deleteProperty(id) {
    ...
    try {
      this.setLoading(true);
      await supabaseManager.deleteProperty(id);
...
document.addEventListener('DOMContentLoaded', function () {
  window.zentroAdmin = new ZentroAdminIntegrated();
});